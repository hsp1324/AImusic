...
class Beat:
	def __init__(self, process, loop, measure_beat_size=1):
		...
		self.loop = loop
		self.measure_beat_size = measure_beat_size
	...
	def repeat_beat(self, loop_index, repeat_range=0):
		...
		for i in range(len(self.process)):
			...
			if(i in range(repeat_range+1)):
				...
				measure_beat = self.generate_relative_beat(loop_index, i, repeat_range)
	...
	# 		if(i == 0 or i == 1):
	# 			measure_beat = self.random_beat(self.measure_beat_size)
	# 		else:
	...
	def random_beat(self):
		...
		measure_beat = []
		probability = [1]*2
		while(accumulated_beat < self.measure_beat_size):
			if(accumulated_beat >= self.measure_beat_size - 1/8):
				next_beat = 8
			else:
	...
	def generate_relative_beat(self, loop_index, index, repeat_range):
		# 반전  check
		# 나누기
		# 세잎단음표
		# 앞붙점 뒷붙점
		...
		if repeat_range == 0:
			...
			original_beat = self.beats[loop_index*len(self.process) + index%repeat_range]
	...
	def reversing_half_beat(self, original_beat):
		...
		for index in range(len(original_beat)):
			accum_beat += 1/original_beat[index]
			if(accum_beat == 1/2):
				can_reverse = True
	...
	def merge_beat(self, beat):
		...
		return_beat = beat[:merging_index] + \
					  [1./(1./beat[merging_index] + 1./beat[merging_index+1])] + \
